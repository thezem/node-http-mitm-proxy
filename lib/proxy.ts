...
export class Proxy implements IProxy {
  ...
  ignore(hosts: string[]) {
    this.ignoreHosts = hosts;
  }
  allowOnly(hosts: string[]) {
    // only prcess the hosts that are in the list
    this.ignoreHosts = ['*'];
    this.allowOnlyHosts = hosts;
  }
  ...
  _createHttpsServer(options: ServerOptions & { hosts?: string[] }, callback: ICreateServerCallback) {
    ...
    httpsServer.listen(listenOptions, () => {
      if (callback) {
        callback((httpsServer.address() as AddressInfo).port, httpsServer, wssServer);
      }
    });
  }
  ...
  _onHttpServerConnect(req: http.IncomingMessage, socket: stream.Duplex, head: Buffer) {
    const self = this;
    ...
    const passThrough =
      self.allowOnlyHosts.length > 0 &&
      !self.allowOnlyHosts.some((pattern) => new RegExp(pattern.replace('*.', '.*')).test(hostname || ''));

    if (passThrough) {
      const serverSocket = net.connect(Number(port), hostname || 'localhost', () => {
        socket.write('HTTP/1.1 200 Connection Established\r\n' + 'Proxy-agent: Node.js-Proxy\r\n' + '\r\n');
        serverSocket.write(head);
        serverSocket.pipe(socket);
        socket.pipe(serverSocket);
      });
      serverSocket.on('error', self._onSocketError.bind(self, 'PROXY_TO_SERVER_SOCKET'));
      console.log('Ignoring host', hostname, 'for port', port);
  ...
  _onHttpServerConnectData(req: http.IncomingMessage, socket: stream.Duplex, head: Buffer) {
    const self = this;
        ...
        async.auto(
          {
            keyFileExists(callback) {
              return fs.exists(files.keyFile, (exists) => callback(null, exists));
            },
            certFileExists(callback) {
              return fs.exists(files.certFile, (exists) => callback(null, exists));
            },
            // @ts-ignore
            httpsOptions,
          },
  ...
  onCertificateRequired(hostname: string, callback: OnCertificateRequiredCallback) {
    const self = this;
    return callback(null, {
  ...
  onCertificateMissing(ctx: ICertficateContext, files: ICertDetails, callback: ErrorCallback) {
  ...
  _onWebSocketServerConnect(isSSL: boolean, ws: WebSocketType, upgradeReq: IncomingMessage) {
    const self = this;
    // @ts-ignore
  ...
  _onHttpServerRequest(isSSL: boolean, clientToProxyRequest: IncomingMessage, proxyToClientResponse: ServerResponse) {
    const self = this;
    const ctx: IContext = {
      uuid: uuid(),
      isSSL,
      ...
      connectRequest:
        self.connectRequests[`${clientToProxyRequest.socket.remotePort}:${clientToProxyRequest.socket.localPort}`] || undefined,
    ...
    function proxyToServerRequestComplete(serverToProxyResponse: http.IncomingMessage) {
  ...
  _onRequestHeaders(ctx: IContext, callback: ErrorCallback) {
    async.forEach(this.onRequestHeadersHandlers, (fn, callback) => fn(ctx, callback), callback);
  }

  _onRequest(ctx: IContext, callback: ErrorCallback) {
    async.forEach(this.onRequestHandlers.concat(ctx.onRequestHandlers), (fn, callback) => fn(ctx, callback), callback);
  }
  ...
  _onWebSocketFrame(ctx: IWebSocketContext, type: string, fromServer: boolean, data: WebSocket.RawData, flags?: WebSocketFlags | boolean) {
    const self = this;
    async.forEach(
      ...
      (err) => {
        ...
        if (destWebSocket.readyState === WebSocket.OPEN) {
          ...
          self._onWebSocketError(
            ctx,
            new Error(
              `Cannot send ${type} because ${fromServer ? 'clientToProxy' : 'proxyToServer'} WebSocket connection state is not OPEN`
            )
          );
        }
      }
  ...
  _onWebSocketClose(ctx: IWebSocketContext, closedByServer: boolean, code: number, message: Buffer) {
    const self = this;
    if (!ctx.closedByServer && !ctx.closedByClient) {
      ...
      async.forEach(
        ...
        (err) => {
          ...
          if (clientToProxyWebSocket.readyState !== proxyToServerWebSocket.readyState) {
            try {
              if (clientToProxyWebSocket.readyState === WebSocket.CLOSED && proxyToServerWebSocket.readyState === WebSocket.OPEN) {
                code === 1005 ? proxyToServerWebSocket.close() : proxyToServerWebSocket.close(code, message);
              } else if (proxyToServerWebSocket.readyState === WebSocket.CLOSED && clientToProxyWebSocket.readyState === WebSocket.OPEN) {
                code === 1005 ? proxyToServerWebSocket.close() : clientToProxyWebSocket.close(code, message);
              }
            } catch (err: any) {
              return self._onWebSocketError(ctx, err);
            }
  ...
  _onWebSocketError(ctx: IWebSocketContext, err: Error) {
    ...
    const clientToProxyWebSocket = ctx.clientToProxyWebSocket!;
    const proxyToServerWebSocket = ctx.proxyToServerWebSocket!;
    if (proxyToServerWebSocket && clientToProxyWebSocket.readyState !== proxyToServerWebSocket.readyState) {
      try {
        if (clientToProxyWebSocket.readyState === WebSocket.CLOSED && proxyToServerWebSocket.readyState === WebSocket.OPEN) {
  ...
  _onResponse(ctx: IContext, callback: ErrorCallback) {
    async.forEach(this.onResponseHandlers.concat(ctx.onResponseHandlers), (fn, callback) => fn(ctx, callback), callback);
  }

  _onResponseHeaders(ctx: IContext, callback: ErrorCallback) {
    async.forEach(this.onResponseHeadersHandlers, (fn, callback) => fn(ctx, callback), callback);
  }
